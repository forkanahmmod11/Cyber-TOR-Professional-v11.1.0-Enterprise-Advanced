...
def banner():
    print("\033[1;36m╔════════════════════════════════════════════════════════════╗")
    print("║\033[1;97m                  CYBER TOR v11.1.0                         \033[1;36m║")
...
def malware_analyze(path):
    ...
    return save_report("malware_analysis",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"analysis":meta})
...
def dns(host, scope):
    ...
    return save_report("dns",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":host,"addresses":ips,"details":rows})
...
def geoip(host, scope):
    ...
    return save_report("geoip",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":host,"result":data})

def tls(host, scope, port=443):
    host=require_scope(host,scope)
    result=tls_posture(host,port,scope,finding)
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":host,"port":port,**result}
...
def web_audit(url, scope, deep=True):
    ...
    findings=[]
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url}
...
def security_headers_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"headers":h,"security_headers":sh,"findings":findings}
...
def adaptive_site_analysis(url, scope):
    ...
    try: article_summary(result, "Cyber Tor Adaptive Technology Analysis")
...
def vulnerability_assessment(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"security_headers":sh,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def broken_link_endpoint_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"links_checked":len(rows),"results":rows,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def sensitive_object_audit(url, scope):
    ...
    for name in SENSITIVE_PATHS:
        ...
        rows.append(row); time.sleep(MIN_DELAY)
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"results":rows,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def error_misconfiguration_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"checks":checks,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def security_configuration_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"configuration":config,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def ssl_certificate_intelligence(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"host":host,"certificate":result.get("certificate",{}),"verification":result.get("verification"),"protocols":result.get("protocols",{}),"errors":result.get("errors",[]),"findings":result.get("findings",[])}
...
def robots_sitemap_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"results":results,"findings":findings}; score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def redirect_security_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"chain":chain,"final_url":current,"findings":findings}; score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def subdomain_intelligence(url, scope):
    url = normalize_url(url); host = require_scope(url, scope)
    data = {"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"host":host,"subdomains":[],"sources":[]}
    candidates = set()
    try:
...
def api_security_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'status':r.status,'api_endpoints':endpoints,'findings':findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def cookie_security_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'cookies':rows,'findings':findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def cors_security_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'cors':{k:v for k,v in h.items() if k.startswith('access-control-')},'findings':findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def javascript_security_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'scripts':sorted(set(scripts))[:100],'indicators':indicators,'findings':[]}
...
def cloud_exposure_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'cloud_references':sorted(set(matches))[:50],'findings':findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def waf_cdn_detection(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'detected':sorted(set(signals)),'evidence':{k:v for k,v in h.items() if k in ('server','via','cf-ray','x-amz-cf-id','x-sucuri-id','x-cache')}}
...
def authentication_security_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'password_forms':password_forms,'auth_links':auth_links,'findings':findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def api_endpoint_discovery(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'endpoints':sorted(endpoints)[:100]}
...
def dependency_cve_intelligence(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'detected_components':candidates,'note':'This module identifies version candidates; verify against a trusted CVE database before treating a CVE as confirmed.'}
...
def http_method_security_audit(url, scope):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':url,'status':r.status,'allow':allow,'methods':methods,'findings':findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def risk_scoring_engine(target=None):
    ...
    data={'tool':'Cyber Tor','version':VERSION,'timestamp':stamp(),'target':target,'reports_considered':len(selected),'finding_counts':counts,'risk_score':score,'risk_level':level,'security_score':max(0,100-normalized),'grade':grade,'top_findings':findings[:20]}
    print("\n\033[1;35mCYBER TOR SECURITY SCORE\033[0m")
...
def nmap_scan(host, scope, profile="quick"):
    ...
    if not shutil.which("nmap"):
        ...
        return save_report("nmap",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":host,"profile":profile,"engine":"builtin-fallback","results":rows})
    profiles={
    ...
    return save_report("nmap",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),
...
def ports(host,scope,plist=None):
    ...
    return save_report("ports",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":host,"results":rows})
...
def load_check(url,scope,count=10,delay=.5):
    ...
    return save_report("load_check",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"count":count,"delay":delay,"results":rows})

def file_hash(path):
    data=hashes(path)
    print(json.dumps(data,indent=2))
    return save_report("file_hash",{"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),**data})
...
def _collect_findings(obj, module="General", out=None):
    ...
    if isinstance(obj, dict):
        ...
        next_module = module
        if obj.get("tool") == "Cyber Tor" and obj.get("target"):
            next_module = module
...
def article_summary(report, title="Cyber Tor Security Analysis"):
    ...
    lines.append("-----------------")
    if findings:
        lines.append(f"Cyber Tor-এর বিশ্লেষণে মোট {len(findings)}টি security/configuration observation পাওয়া গেছে। সামগ্রিক risk level {level}, এবং weighted risk score {score}।")
...
def article_summary_from_target(target=None):
    ...
    report={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":target or selected[-1].get("target"),"modules":selected}
...
def enterprise_posture_audit(url, scope):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,"engine":"Enterprise Posture Engine 8.0","summary":{"risk_score":score,"risk_level":level,"finding_count":len(findings),"risk_analysis":meta},"modules":reports,"compliance_alignment":compliance_map(findings)}
...
def deep_web_pentest(target, scope, profile="deep-web"):
    ...
    print("\n\033[1;35m=== CYBER TOR SAFE ACTIVE WEB PENTEST ===\033[0m")
    ...
    result.update({"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":url,
...
def deep_analysis(target,scope):
    ...
    print("\n\033[1;35m=== CYBER TOR ENTERPRISE DEEP ANALYSIS ===\033[0m")
    modules=[]
    for label,fn in [
    ...
    try: article_summary(data, "Cyber Tor Enterprise Deep Security Analysis")
    except Exception as e: print("[!] Article summary generation failed:",e)
    return data
...
def module_health_check():
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"active":active,"total":len(rows),"all_active":active==len(rows),"modules":rows}
...
def enterprise_compliance(target=None):
    ...
    data={"tool":"Cyber Tor","version":VERSION,"timestamp":stamp(),"target":target,"controls":compliance_matrix(findings)}; print(json.dumps(data,indent=2,ensure_ascii=False)); return save_report("enterprise_compliance",data)
...
def url_first_menu():
    ...
    print("\n" + "=" * 64)
    print(" CYBER TOR — URL-FIRST ANALYSIS")
    print("=" * 64)
...
def ransomware_guard(path, interval=2.0, once=False):
    ...
    print("[+] Cyber Tor ransomware guard active. Detection/alerting only; no destructive remediation."); return monitor(path, interval, once)
...
def main():
    p=argparse.ArgumentParser(description="Cyber Tor v11.0 - enterprise security assessment, exposure management and defensive platform")
