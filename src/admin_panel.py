#!/usr/bin/env python3
"""Cyber Tor Professional v7.2 local control plane.
...
VERSION='11.0.0'; DEFAULT_USERNAME='admin'; BASE=Path(__file__).resolve().parents[1]; STATE=BASE/'state'; REPORTS=BASE/'reports'; CONFIG=STATE/'admin.json'; AUDIT=STATE/'audit.log'
...
def cfg():
    ...
    c.setdefault('mfa_policy',{'required_for_roles':['super_admin','security_admin'],'issuer':'Cyber TOR Professional'})
    return c
def save(c):
...
<link rel="icon" type="image/png" href="/assets/cyber-tor-mark.png">
<title>{html.escape(t)} · Cyber Tor Professional</title>
<style>
...
<header><div class="brand"><div class="brand-mark"><img src="/assets/cyber-tor-mark.png" alt="Cyber Tor"></div><div><div class="brand-title">CYBER TOR PROFESSIONAL</div><div class="brand-sub">v{VERSION} · Enterprise Security Operations Center</div></div></div>{nav}</header><main>{b}</main>"""
...
class H(BaseHTTPRequestHandler):
    ...
    def auth(self):
        ...
        if isinstance(x,dict) and x.get('exp',0)>time.time() and x.get('authenticated',False): return x
    ...
    def do_GET(self):
        ...
        if not c and u.path not in ('/setup','/assets/cyber-tor-mark.png'): return self.sendx(302,'',{'Location':'/setup'})
        if u.path=='/assets/cyber-tor-mark.png':
            try:
                data=(ASSETS/'cyber-tor-mark.png').read_bytes()
    ...
    def do_POST(self):
        if self.path.startswith("/api/v1/"): return self.do_API()
        c=setup(); d=self.form(); a=self.auth()
        if self.path=='/setup':
        ...
        if self.path=='/login':
            ...
            if verify(c,u_name,p):
                ...
                if r.get('mfa_enabled'):
                    ...
                    body=f'''<div class=login-shell><div class=card login-card><div class=login-logo><img src="/assets/cyber-tor-mark.png"></div><div class=login-title>CYBER <span>TOR</span></div><div class=login-tag>MULTI-FACTOR VERIFICATION</div><h2>Enter authenticator code</h2><p class=muted>Enter the current 6-digit code from your authenticator.</p><form method=post action=/login/mfa><input name=code inputmode=numeric autocomplete=one-time-code maxlength=6 pattern="[0-9]{{6}}" placeholder="6-digit code" required style="width:100%"><input type=hidden name=csrf value="{SESSIONS[sid]['csrf']}"><button style="width:100%;padding:13px">Verify MFA</button></form></div></div>'''
                ...
                loader='''<div class=loading-shell><div class=loader-card><div class=loader-logo><img src="/assets/cyber-tor-mark.png" alt="Cyber Tor"></div><div class=loader-ring></div><div class=loader-title>Initializing Cyber Tor</div><div class=loader-sub>Verifying secure session · Loading control plane · Preparing operations center</div><div class=progress></div></div></div><script>setTimeout(function(){window.location.replace('/');},3500);</script>'''
                return self.sendx(200,page('Initializing Cyber Tor',loader),{'Set-Cookie':f'sid={raw}; HttpOnly; SameSite=Strict; Path=/; Max-Age={SESSION_SECONDS}'})
        ...
        if self.path=='/assessments/run':
            ...
            cmd=[sys.executable,str(BASE/'src'/'cyber_tor.py'),'--scope',str(scope_path)]
        ...
        if self.path=='/files/run':
            ...
            import subprocess
            cmd=[sys.executable,str(BASE/'src'/'cyber_tor.py'),module,target]
...
def run(host='127.0.0.1',port=8787): setup(); print(f'Cyber Tor v{VERSION} admin: http://{host}:{port}/'); ThreadingHTTPServer((host,port),H).serve_forever()
