...
def baseline(root):
    root=Path(root).expanduser().resolve()
    data={"tool":"Cyber Tor Professional","version":"11.0.0","created_at":stamp(),"root":str(root),"files":inventory(root)}
...
def scan_file(path, quarantine=False):
    ...
    result={"tool":"Cyber Tor Professional","version":"11.0.0","timestamp":stamp(),"path":str(p),"sha256":sha256(p),"entropy":round(entropy(p),4),"findings":findings,"disposition":"review" if findings else "no_indicators_observed"}
...
def compare(root):
    ...
    return {"tool":"Cyber Tor Professional","version":"11.0.0","timestamp":stamp(),"root":str(root),"added":added,"removed":removed,"changed":changed,"suspicious":suspicious,"ransomware_signal":len(suspicious)>=3 or any(i.get("indicator")=="ransomware-like extension" for i in suspicious)}
